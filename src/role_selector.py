...
- Background/extra work or generic day-player roles — this includes: roles named with plurals or generic titles instead of character names (e.g., "BODYGUARDS", "WEDDING GUESTS", "STAFF", "BANQUET GUESTS", "FEMALE GUESTS 1 AND 2", "POLICE MAN", "MAID"); roles described as group/ensemble with no individual character identity; roles where the actor is essentially atmosphere/background with no lines or character arc (e.g., "PARTYGOERS", "CROWD", "MASKED AUDIENCE MEMBERS"). A role must have a UNIQUE CHARACTER NAME (e.g., "JACE", "DETECTIVE MORRIS", "DR. CHEN") to be considered — generic functional titles like "WAITER", "CLERK", "NURSE", "REPORTER", "BODYGUARD" are background work even if labeled "day player." EXCEPTION: In commercials, roles labeled "featured", "featured talent", "principal", or "hero" where the talent is prominently on-camera ARE legitimate commercial bookings even without dialogue — do NOT reject these. Only reject commercial roles that are clearly crowd/atmosphere with no camera focus.
