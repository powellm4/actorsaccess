...
ACTOR_PROFILE = """
- Appears 25 but people often think he's younger; plays 17-30 convincingly (yes, including 17-22 college-aged roles)
...
_MEDIUM_DRIVE_KEYWORDS = [
    "las vegas", "vegas", "san francisco", "oakland", "san jose", "bay area",
    "phoenix", "tucson", "arizona", "scottsdale", "tempe", "mesa, az", "reno",
    "sacramento", "fresno", "stockton", "modesto",
]
_FLY_TO_KEYWORDS = [
...
- Medium drive (San Francisco, Bay Area, Las Vegas, Phoenix, Tucson, Arizona generally, Sacramento, NV/AZ): SKIP if total pay is under $500
...
- "Total pay" means the full amount for the job, not per day. If the listing says "$200/day" for a 3-day shoot, total pay is $600 — that passes the short-drive $250 threshold but not the medium-drive $500 threshold. For hourly rates, assume an 8-hour day (e.g., "$150/hour" = $1,200/day). If the total pay clearly exceeds the threshold, do NOT reject
...
- ABSOLUTELY NEVER include acting skills, improv training, UCB, Groundlings, dance experience, guitar, or ANY training/experience in submission notes. These are NEVER relevant to submission notes regardless of what the casting asks. If a casting says "let us know about your experience" — respond with ACTION: SUBMIT (no note). The actor's profile/resume already covers this. NARROW EXCEPTION: if the casting explicitly asks for footage or clips demonstrating a SPECIFIC physical skill (e.g., "submit dance clips", "include skating footage"), AND that exact skill IS listed in the actor profile, you MAY include a one-sentence note stating the relevant experience — this is the ONLY case where experience belongs in the note. If the skill is not in the actor profile, do NOT include a note (see physical-skill footage rule below).
