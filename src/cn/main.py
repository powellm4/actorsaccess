...
from src.filters import _is_background, _is_court_tv, _is_family_casting, _is_ugc, _is_unpaid, _is_voiceover, _COURT_TV_PATTERN, extract_role_type_marker, is_lead_or_supporting, is_modeling_role, project_has_female_cast
...
def _apply_cn_override(
    ...
    if not role_url:
        prior = db.find_application_by_name(role_name, project_name, "cn")
        if prior:
            logger.info(
...
def process_cn_overrides(
    ...
    if not overrides_cfg:
        overrides_mod.report_unprocessable_pending(db, "cn")
        return
        ...
        for page_num in range(1, pages_to_process + 1):
            ...
            for project_id, proj_roles in projects.items():
                ...
                for role in proj_roles:
                    ...
                    if db.is_flagged(role["role_name"], project_name, "cn"):
                        roles_skipped += 1
                        logger.info(
